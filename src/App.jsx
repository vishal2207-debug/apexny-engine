  ...
  return (
    <div className="min-h-screen bg-[#05070b] text-slate-200 p-3 md:p-5 font-sans selection:bg-emerald-500 selection:text-black">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="bg-[#090d16] p-4 rounded-xl border border-slate-800 shadow-xl space-y-3">
            <div className="flex justify-between items-center px-1">
              <span className="text-xs font-bold text-slate-400 font-mono tracking-wider">WATCHLIST ({INSTITUTIONAL_ASSETS.length} ASSETS)</span>
                ...
                <div className="bg-[#090d16] p-4 rounded-xl border border-slate-800 shadow-xl space-y-3">
                  ...
                  <div className="bg-[#0d1322] p-3 rounded-lg border border-slate-800/80 space-y-2 text-xs">
                    <div className="flex justify-between items-center">
                      <span className="text-cyan-400 font-mono font-bold uppercase tracking-wider">Zone: {activeStructure.smcZone}</span>
                      <span className="text-[10px] font-mono text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">Both Side Ready • No Repaint</span>
                    </div>
                    <p className="text-slate-300 font-sans leading-relaxed"><strong className="text-white">Execution Logic:</strong> {activeStructure.logic}</p>
                  </div>
                </div>

                <div className="bg-[#090d16] p-4 rounded-xl border border-slate-800 shadow-xl space-y-3">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                    <div className="flex items-center space-x-2"><span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse" /><span className="text-xs font-bold text-white uppercase tracking-wider font-mono">MACRO IMPACT SCENARIOS FOR {selectedAsset.symbol}</span></div>
                    <span className="text-[10px] font-mono text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/20">{selectedAsset.newsType} FEED ACTIVE</span>
                  </div>
                  ...
                  <div className="bg-[#0d1322] p-3.5 rounded-lg border border-slate-800 text-xs space-y-2">
                    <div className="flex justify-between items-center"><span className="font-bold text-white">Impact Analysis: {selectedNews.event}</span><span className="text-[10px] font-mono text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">{selectedNews.status}</span></div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs pt-1">
                      <div className="p-2.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-300"><span className="font-bold text-emerald-400 block font-mono">Bullish Scenario:</span>{selectedNews.scenarioBullish}</div>
                      <div className="p-2.5 rounded bg-rose-500/10 border border-rose-500/20 text-rose-300"><span className="font-bold text-rose-400 block font-mono">Bearish Scenario:</span>{selectedNews.scenarioBearish}</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {terminalMode === 'OPTIONS' && (
              <div className="space-y-4">
                ...
                <div className="bg-[#090d16] p-4 rounded-xl border border-slate-800 shadow-xl space-y-2">
                  <div className="text-xs font-bold text-white font-mono uppercase tracking-wider">EXECUTION LEGS (DELTA EXCHANGE)</div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs font-mono text-left">
                      <thead><tr className="text-slate-500 border-b border-slate-800"><th className="pb-2">ACTION</th><th className="pb-2">STRIKE</th><th className="pb-2">TYPE</th><th className="pb-2">PREMIUM</th></tr></thead>
      ...
      {activeTab === 'chart' && (
        <div className="space-y-3">
          <div className="bg-[#090d16] p-3 rounded-xl border border-slate-800 flex flex-wrap items-center justify-between gap-3 shadow-lg">
            <div className="flex items-center space-x-3">
              <button onClick={() => setActiveTab('dashboard')} className="bg-slate-800 hover:bg-slate-700 text-white px-3 py-1.5 rounded-lg text-xs font-mono font-bold transition flex items-center gap-1.5">← Back to Dashboard</button>
      ...
      <footer className="mt-4 p-3 bg-[#090d16] rounded-xl border border-slate-800/80 flex flex-wrap justify-between items-center text-xs text-slate-400 gap-2">
        <div className="flex items-center gap-2 font-mono"><span className="w-2 h-2 rounded-full bg-emerald-400 inline-block"></span><span>Designed by Mr. Vishal Langade • Proudly Indian</span></div>
        <div className="font-mono text-emerald-400 font-bold">Crafted with Precision by Mr. Vishal Langade</div>
      </footer>
    </div>
  );
}